...
- Spools can now use a Silk surface finish, alongside Matte, Standard and
  Gloss, selectable in the add and edit forms and accounted for the same way
  in colour search.
...
- Spools now carry a surface finish (Matte / Standard / Gloss), selectable in the add and edit forms and shown as a badge next to the colour swatch. Existing spools read back as Standard, so no data migration is needed.
- Colour search accounts for finish: a matte spool is compared as slightly desaturated and brighter, a gloss spool as more saturated and slightly darker, which is closer to how the filament actually prints. Standard spools are unaffected.
...
- Docker images cross-compile the server natively and install target libc headers, so multi-arch builds succeed.
- Docker build context excludes `target`, `.git` and worktrees.
- Images are published on `v*` tags only; the `edge` tag is dropped.
...
- Playwright test suite (`tests/playwright/`) covering navigation, filament list/detail/create/edit, spool list/detail/create/edit/clone, and API schema validation (38 tests, all passing).
...
- CSS styling for the app shell, sidebar, data tables, buttons, and dark-mode overrides, defined in `style/spoolman.css` and bundled via `Leptos.toml` `style-file`. The app is no longer completely unstyled (fixes B4).
...
[1.3.0]: https://github.com/crjeder/Spoolman-light/compare/v1.2.0...v1.3.0
[1.2.0]: https://github.com/crjeder/Spoolman-light/compare/v1.1.0...v1.2.0
[1.1.0]: https://github.com/crjeder/Spoolman-light/compare/v1.0.0...v1.1.0
