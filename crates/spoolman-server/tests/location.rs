...
async fn get_location_returns_200_with_spool_count() {
    let (app, _dir) = common::make_app();
    let (_, created) = common::request(&app, Method::POST, "/api/v1/location", Some(json!({ "name": "Shelf B" }))).await;
    let lid = created["id"].as_u64().unwrap();

    let (status, body) = common::request(&app, Method::GET, &format!("/api/v1/location/{lid}"), None).await;
...
async fn list_locations_returns_created() {
    let (app, _dir) = common::make_app();
    common::request(&app, Method::POST, "/api/v1/location", Some(json!({ "name": "Drawer 1" }))).await;
...
async fn update_location_name_returns_200() {
    let (app, _dir) = common::make_app();
    let (_, created) = common::request(&app, Method::POST, "/api/v1/location", Some(json!({ "name": "Old Name" }))).await;
    let lid = created["id"].as_u64().unwrap();
...
async fn delete_location_returns_204() {
    let (app, _dir) = common::make_app();
    let (_, created) = common::request(&app, Method::POST, "/api/v1/location", Some(json!({ "name": "Temp" }))).await;
    let lid = created["id"].as_u64().unwrap();

    let (status, _) = common::request(&app, Method::DELETE, &format!("/api/v1/location/{lid}"), None).await;
...
async fn delete_location_blocked_by_spool_returns_409() {
    ...
    let (_, loc) = common::request(&app, Method::POST, "/api/v1/location", Some(json!({ "name": "In Use" }))).await;
