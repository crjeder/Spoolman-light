...
Defines the visual styling of the Leptos WASM frontend: layout, dark mode, data tables, color swatches, buttons, and how CSS is generated and bundled at build time. Styling is implemented as a plain CSS file (`style/spoolman.css`), not the `stylers` proc-macro originally proposed for this capability — see the archived `add-css-styling-stylers` change for why.
...
### Requirement: CSS is bundled at build time
The system SHALL define styles in `style/spoolman.css`, referenced via the `style-file` setting in `Leptos.toml`, which SHALL be bundled into `spoolman-server.css` by `cargo-leptos` at build time.
