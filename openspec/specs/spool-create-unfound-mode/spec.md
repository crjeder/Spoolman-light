# Capability: Spool Create Unfound Mode

## Purpose

On the Spool Create form, falls back to a manual "unfound" mode when the SpoolmanDB search finds no matching entry: disables the filament dropdown and pre-fills the spool name from the search text, so the user isn't blocked from creating a spool for filament that isn't in the database.
